...
public:
    ...
    // 设置和获取Int型参数，如 Width和Height，详细内容参考SDK安装目录下的 CameraNode.xlsx 文件
    int     GetIntValue(IN const char* strKey, OUT unsigned int *pnValue);
    int     SetIntValue(IN const char* strKey, IN unsigned int nValue);

    // 设置和获取Float型参数，如 ExposureTime和Gain，详细内容参考SDK安装目录下的 CameraNode.xlsx 文件
    int     GetFloatValue(IN const char* strKey, OUT float *pfValue);
    int     SetFloatValue(IN const char* strKey, IN float fValue);

    // 设置和获取Enum型参数，如 PixelFormat，详细内容参考SDK安装目录下的 CameraNode.xlsx 文件
    int     GetEnumValue(IN const char* strKey, OUT unsigned int *pnValue);
    int     SetEnumValue(IN const char* strKey, IN unsigned int nValue);

    // 设置和获取Bool型参数，如 ReverseX，详细内容参考SDK安装目录下的 CameraNode.xlsx 文件
    int     GetBoolValue(IN const char* strKey, OUT bool *pbValue);
    int     SetBoolValue(IN const char* strKey, IN bool bValue);

    // 设置和获取String型参数，如 DeviceUserID，详细内容参考SDK安装目录下的 CameraNode.xlsx 文件UserSetSave
